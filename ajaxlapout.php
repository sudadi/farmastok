			...
			<tbody>
				...
				if ($hasil) {
				  while($baris = $hasil->fetch_array(MYSQLI_ASSOC)) {
					...
					$sql="SELECT tobatout.qty, tobat.kdobat, tobat.nmobat FROM tobatout INNER JOIN tobat on tobatout.kdobat=tobat.kdobat WHERE kdtrans='$kdtrans'";
					$result=$db->query($sql);
						$n=$result->num_rows+1;
					echo "<tr><td rowspan='".$n."'>".$kdtrans."</td>";
					echo "<td colspan='3'> Depo/Satelit : ".$baris['nmsat']."  Tgl. : ".$baris['tgltrans']."</td></tr>";
